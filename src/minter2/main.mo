...
actor class DRC721(_name : Text, _symbol : Text) {
    ...
    public func allCollections() : async [(Nat, Text)] {
        let ans =Iter.toArray<(Nat, Text)>(tokenURIs.entries());
        ans
    };

    public shared ({caller}) func myCollection() : async [(Nat, Text)] {
        let keys = tokenURIs.keys();
        var ans = Buffer.Buffer<(Nat, Text)>(0);
        for(key in keys){
            let p = owners.get(key);
            switch(p){
                case (null){};
                case (_) {
                    let p_unwrap = Option.get(p, caller);
                    if(Principal.equal(p_unwrap, caller)){
                        let uri = tokenURIs.get(key);
                        ans.add((key, Option.get(uri, "")));
                    };
                };
            };
        };
